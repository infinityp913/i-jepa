...
class Tokenizer(nn.Module):
    ...
    def __init__(self, img_size, patch_size):
        ...
        if isinstance(img_size, int):
            ...
            output_size = tuple(int(x) for x in img_size)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, masks=None):
        ...
        Args:
            ...
            masks (torch.Tensor): Patch indices to keep [B, n_context]. Pass None to use all context patches.
        Returns:
...
class Predictor(nn.Module):
    ...
    def forward(self, x, x_masks, y_masks):
        ...
        Args:
            x (torch.Tensor):             Context encoder outputs [B, n_context, n_embed].
...
class ViT(nn.Module):
    ...
    def __init__(self, num_classes, num_patches=196, patch_size=16, img_channels=3, d_model=768, n_head=12, n_layers=12):
        """
        Initializes the ViT.
        ...
        self.feature_extractor = Encoder(
            num_patches, patch_size=patch_size, img_channels=img_channels, d_model=d_model, n_head=n_head, n_layers=n_layers
        )
